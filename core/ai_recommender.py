...
#         # 1. IA apenas para preço e tipo de veículo (não para origem/destino)
#         prompt = f"""
# Extraia do texto do usuário APENAS os campos de preço máximo e tipo de veículo.
# Texto: "{text}"

# Campos:
# - "price_max": valor numérico (se mencionado)
# - "vehicle_type": "carro" ou "moto" (se mencionado)
# - "start_time_after": data/hora (se mencionado)

# Não inclua "origin" nem "destination".
...
#             # Atualiza apenas os campos que a IA retornou
#             for key in ['price_max', 'vehicle_type', 'start_time_after']:
#                 if key in ai_filters:
...
#         # 3. Extrai origem (sempre via regex)
...
#         # 4. Extrai destino (sempre via regex)
...
#         # 5. Fallback para preço (caso a IA não tenha capturado)
...
#         # 6. Fallback para tipo de veículo
...
#         # 7. Tratamento especial: se destination for "Carro" ou "Moto", mover para vehicle_type
#         if 'destination' in filters and filters['destination'].lower() in ['carro', 'moto']:
#             filters['vehicle_type'] = filters['destination'].lower()
#             del filters['destination']

#         # 8. Remove campos vazios
...
#     def extract_filters(self, text: str) -> dict:
#         import re
#         # Inicializa com todos os campos possíveis (None por padrão)
#         filters = {
#             'origin': None,
#             'destination': None,
#             'price_min': None,
...
#         # 1. IA extrai todos os campos (com null para não mencionados)
#         prompt = f"""
...
#                     raw = raw[start:end+1]
#             ai_filters = json.loads(raw)
#             # Atualiza apenas os campos que a IA retornou (não nulos)
...
#         # 7. Fallback para vehicle_type (se não veio da IA)
#         if not filters.get('vehicle_type'):
#             if re.search(r'\bcarro\b', text, re.IGNORECASE):
#                 filters['vehicle_type'] = 'carro'
#             elif re.search(r'\bmoto\b', text, re.IGNORECASE):
...
#         # 8. Se destination for "Carro" ou "Moto", mover para vehicle_type
#         if filters.get('destination') and filters['destination'].lower() in ['carro', 'moto']:
#             filters['vehicle_type'] = filters['destination'].lower()
#             filters['destination'] = None

#         # 9. Remove valores vazios (mas mantém None para campos não encontrados)
#         # Não removemos None para padronizar a saída
#         logger.info(f"Filtros extraídos (final): {filters}")
#         return filters
